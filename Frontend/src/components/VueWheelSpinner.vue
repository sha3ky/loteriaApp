...
async function loadFont() {
  const font = new FontFace(
    "Special Elite",
    "url(https://fonts.gstatic.com/s/specialelite/v18/XLYgIZbkc4JPUL5CVArUVL0ntnAOSFNuQsI.woff2)"
  );
  await font.load();
  document.fonts.add(font);
}

// Llama a esta función antes de dibujar en el canvas
onMounted(async () => {
  await loadFont(); // Cargar la fuente antes de dibujar
  drawWheel(); // Llama a drawWheel después de que la fuente esté disponible
});
// ********************************************************
...
function drawLabel(
  ...
  // context.font = "bold 12px Arial"; // Cambia "16px" por un valor menor, por ejemplo "12px"
  // context.font = "10px sans-serif";
  context.font = " 10px 'Special Elite', Arial";
...
function spinWheel(winnerIndex) {
  console.log("vueWhellSpinner", winnerIndex);
  ...
  // Get random spins count between 1 and 99
  const extraSpins = getRandomBetween(1, 50);

  // Calculate the extra spins angle
  const extraSpinsAngle = extraSpins * 360;

  // Get the winner segment's end angle
  const { endAngle: winnerEndAngle } = getSliceAngles(
    winnerIndex,
    currentAngle.value
  );
  // Calculate the target angle, adding a slight random offset within the slice
  // const randomOffset = getRandomBetween(
  //   -getAnglePerSlice() / 2,
  //   getAnglePerSlice() / 2
  // );
  // const targetAngle =
 ...
 const targetAngle =
  ...
  extraSpinsAngle +
  (getCursorAngle() - winnerEndAngle) +
  (getAnglePerSlice() / 2); // Siempre apunta al CENTRO del segmento
  //-----------------------------------------
  ...
  //     // Play winning sound if available
  //     if (wonAudio.value) {
  //       wonAudio.value.play();
  //     }

  //     // Emit spin end event with the winner index
  ...
  const animate = (currentTime) => {
    ...
    // Reinicia el sonido de "spinning" si terminó y el giro aún no ha terminado
    if (spinningAudio.value && spinningAudio.value.ended) {
      spinningAudio.value.currentTime = 0; // Reinicia el sonido
    ...
    if (progress < 1) {
      ...
      // Normaliza el ángulo de rotación
      rotationAngle = getNormalizedAngle(rotationAngle);
      canvas.style.transform = `rotate3d(0, 0, 1, ${rotationAngle}deg)`;
      currentAngle.value = rotationAngle;

      // Establece isSpinning a false para indicar que el giro ha terminado
      isSpinning.value = false;

      // Reproduce el sonido de ganancia, si está disponible
      if (wonAudio.value) {
        wonAudio.value.play();
      }

      // Emite el evento "spin-end" con el índice del ganador
      emits("spin-end", winnerIndex);

      // Detén el sonido de "spinning" al finalizar el giro
      if (spinningAudio.value) {
        stopAudio(spinningAudio.value);
      }
    }
